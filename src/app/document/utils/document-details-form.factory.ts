...
export function createDocumentDetailsSectionForm(): DocumentCreateDetailsFormGroup {
  return new FormGroup({
    ...
    description: new FormControl<string | null>(null, [Validators.maxLength(4000)]),
    involvement: new FormControl<string | null>(null, [Validators.maxLength(255)]),
    objectReferenceType: new FormControl<string | null>(null, [Validators.maxLength(255)]),
    objectReferenceId: new FormControl<string | null>(null, [Validators.maxLength(255)])
  })
}
...
export function patchDocumentDetailsForm(formGroup: DocumentDetailsFormGroup, details?: DocumentDetail): void {
  ...
  setCharacteristicsOnForm(formGroup, Array.from(details?.characteristics || []))
}

export function setAttachmentsOnForm(formGroup: DocumentDetailsFormGroup, attachments: Attachment[]): void {
...
export function setCharacteristicsOnForm(
  ...
  characteristics.forEach((char) => charsFormArray.push(createCharacteristicFormGroup(char)))
}

export function getAttachmentFormArray(formGroup: DocumentDetailsFormGroup): FormArray<DocumentAttachmentFormGroup> {
...
export function removeCharacteristic(formGroup: DocumentDetailsFormGroup, index: number) {
  const existingChars = getCharacteristicsFormArray(formGroup)
...
export function createCharacteristicFormGroup(
  ...
  return new FormGroup({
    ...
    name: new FormControl(characteristic?.name || null, [Validators.required, Validators.maxLength(255)]),
    value: new FormControl(characteristic?.value || null, [Validators.required, Validators.maxLength(255)])
  })
}

function createAttachmentFormGroup(attachment: Attachment): DocumentAttachmentFormGroup {
  return new FormGroup({
    ...
    storageUploadStatus: new FormControl<boolean>(attachment.storageUploadStatus ?? false, { nonNullable: true })
  })
}
