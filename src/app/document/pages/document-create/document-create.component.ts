...
export class DocumentCreateComponent implements OnInit {
  ...
  constructor(
    ...
    this.canGoNextFromAttachments$ = this.store.select(selectCanGoNextFromAttachments)
  }
  ...
  ngOnInit(): void {
    ...
    this.stepsModel = [
      ...
      {
        label: this.translateService.instant('DOCUMENT_CREATE.STEPPER.ATTACHMENTS')
      },
      {
        label: this.translateService.instant('DOCUMENT_CREATE.STEPPER.CHARACTERISTICS')
      }
    ]
  }

  onDetailsNext(vm: DocumentCreateViewModel, details: Partial<DocumentCreateDetailsStepData>): void {
    this.store.dispatch(DocumentCreateActions.detailsStepPatched({ details }))
    this.store.dispatch(DocumentCreateActions.goToNextStep({ currentStep: vm.activeStep }))
  }

  onAttachmentsBack(vm: DocumentCreateViewModel, attachments: AttachmentDraft[]): void {
    this.store.dispatch(DocumentCreateActions.attachmentsStepPatched({ attachments }))
    this.store.dispatch(DocumentCreateActions.goToPreviousStep({ currentStep: vm.activeStep }))
  }

  onAttachmentsNext(vm: DocumentCreateViewModel, attachments: AttachmentDraft[]): void {
    this.store.dispatch(DocumentCreateActions.attachmentsStepPatched({ attachments }))
    this.store.dispatch(DocumentCreateActions.goToNextStep({ currentStep: vm.activeStep }))
  }

  onAttachmentMimeTypeNotSupported(fileName: string): void {
    this.store.dispatch(DocumentCreateActions.attachmentMimeTypeNotSupported({ fileName }))
  }
  ...
  onCharacteristicsSave(characteristics: DocumentCharacteristicFormValue[]): void {
    this.store.dispatch(DocumentCreateActions.characteristicsStepPatched({ characteristics }))
