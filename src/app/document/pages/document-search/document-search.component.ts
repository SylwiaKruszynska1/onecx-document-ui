...
export class DocumentSearchComponent implements OnInit {
  ...
  resultComponentStateChanged(state: InteractiveDataViewComponentState) {
    this.store.dispatch(DocumentSearchActions.resultComponentStateChanged(state))
  }

  searchHeaderComponentStateChanged(state: SearchHeaderComponentState) {
    this.store.dispatch(DocumentSearchActions.searchHeaderComponentStateChanged(state))
  }

  search(formValue: FormGroup) {
    const searchCriteria = buildSearchCriteria(formValue.getRawValue(), this.criteriaComponent.calendars, {
      removeNullValues: true
    })
    this.store.dispatch(DocumentSearchActions.searchButtonClicked({ searchCriteria }))
  }
  ...
  onDelete(data: RowListGridData) {
    this.store.dispatch(DocumentSearchActions.deleteButtonClicked({ id: data.id as string }))
  }
  ...
  private buildSearchFormGroup(): FormGroup {
    return this.formBuilder.group({
      ...(Object.fromEntries(documentSearchCriteriasSchema.keyof().options.map((k) => [k, null])) as Record<
        keyof DocumentSearchCriteriaSchema,
        unknown
      >)
