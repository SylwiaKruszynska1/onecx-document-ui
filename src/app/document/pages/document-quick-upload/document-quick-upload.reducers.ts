...
export const documentQuickUploadReducer = createReducer(
  initialState,
  on(DocumentCreateOperationsActions.ensureReferenceDataLoaded, (state): DocumentQuickUploadState => ({
    ...state,
    optionsLoading: !state.mimeTypesLoaded && !state.documentTypesLoaded
  })),
  on(DocumentCreateOperationsActions.availableDocumentTypesReceived, (state): DocumentQuickUploadState => ({
    ...state,
    documentTypesLoaded: true,
    optionsLoading: !state.mimeTypesLoaded
  })),
  on(DocumentCreateOperationsActions.availableMimeTypesReceived, (state): DocumentQuickUploadState => ({
    ...state,
    mimeTypesLoaded: true,
    optionsLoading: !state.documentTypesLoaded
  })),
  on(DocumentCreateOperationsActions.startDocumentCreation, (state): DocumentQuickUploadState => ({
    ...state,
    optionsLoading: true
  })),
  on(
