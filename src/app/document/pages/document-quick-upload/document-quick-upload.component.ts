...
export class DocumentQuickUploadComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subs.add(
      this.store.select(documentQuickUploadSelectors.selectOptionsLoading).subscribe((isLoading) => {
  ...
  onSave(): void {
    const createRequest: DocumentCreateUpdate = {
      ...
      lifeCycleState: this.documentQuickUploadForm.controls['lifeCycleState'].value,
      channel: {
