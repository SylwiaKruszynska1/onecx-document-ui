...
export class DocumentQuickUploadFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.documentQuickUploadForm = new FormGroup({
      documentName: new FormControl('', [Validators.required, noSpecialCharacters]),
   ...
   */
  trimSpaceOnPaste(event: ClipboardEvent, controlName: string, maxlength: number) {
    this.documentQuickUploadForm = trimSpaces(event, controlName, this.documentQuickUploadForm, maxlength)
  }
  /**
  ...
  preventSpace(event: KeyboardEvent): void {
    const target = event.target as HTMLInputElement
    if (target.selectionStart === 0 && event.code === 'Space') event.preventDefault()
  }
  ...
  private loadDocumentStatus(): void {
    ...
    const docStatusDraft = this.documentStatus.filter((document) => document.value.toLowerCase() == 'draft')
    if (docStatusDraft.length > 0) {
      this.documentQuickUploadForm.controls['lifeCycleState'].patchValue(this.documentStatus[1].value)
    }
  }
  ...
  private enterDataToListView(file: File) {
    ...
    this.supportedMimeType = this.supportedMimeType?.length ? this.supportedMimeType : []
  ...
  private validateAttachmentArray(): void {
    if (this.attachmentArray.length) {
      const invalidAttachment = this.attachmentArray.filter((attachment) => !attachment.isValid)
