...
export class DocumentDetailsEffects {
  ...
  loadDocumentById$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      switchMap(({ id }) =>
        this.documentService.getDocumentById({ id: id ?? '' }).pipe(
          map((details) =>
  ...
  saveButtonClicked$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      concatLatestFrom(() => this.store.select(documentDetailsSelectors.selectDetails)),
      switchMap(([{ details }, prevDetails]) => {
        ...
        return this.documentService.updateDocument({ id: itemToEditId, documentCreateUpdate: itemToEdit }).pipe(
          mergeMap((response) => [
  ...
  deleteButtonClicked$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      switchMap(([dialogResult, itemToDelete]) => {
        ...
        return this.documentService.deleteDocumentById({ id: itemToDelete.id as string }).pipe(
          map(() => {
            this.messageService.success({
  ...
  startAttachmentDownload$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      switchMap((action) =>
        this.documentService.getFile({ attachmentId: action.attachmentId }).pipe(
          map((response) =>
          ...
          catchError(() => of(DocumentDetailsActions.attachmentDownloadFailed()))
        )
      )
    )
  })
  ...
  documentCreationSuccess$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      map(([action]) => DocumentDetailsActions.navigatedToDetailsPage({ id: action.documentId }))
    )
  })
  ...
  private getUpdateRequest(prevState: DocumentDetail, formValue: DocumentDetailsFormValue): DocumentCreateUpdate {
    return {
      ...
      documentRelationships: (prevState.documentRelationships ? Array.from(prevState.documentRelationships) : []).map(
        (relationship) => ({
          id: relationship.id,
      ...
      relatedParties: (prevState.relatedParties ? Array.from(prevState.relatedParties) : []).map((relatedParty) => ({
        id: relatedParty.id,
      ...
      relatedObject: prevState.relatedObject
        ...
        : undefined,
      categories: (prevState.categories ? Array.from(prevState.categories) : []).map((category) => ({
        id: category.id,
        name: category.name,
        categoryVersion: category.categoryVersion
      })),
      attachments: this.mapAttachments(prevState.attachments || [], formValue.attachments)
    }
  }
  ...
  private mapAttachments(
    ...
    const attachmentMap = formValue.reduce(
      (prev, curr) => {
        prev[curr.id!] = curr
        return prev
      },
  ...
  private mapCharacteristics(formValue: DocumentCharacteristicFormValue[]): DocumentCharacteristicCreateUpdate[] {
