...
describe('DocumentDetailsEffects', () => {
  ...
  describe('navigatedToDetailsPage$', () => {
    it('should dispatch ensureReferenceDataLoaded and navigatedToDetailsPage when navigating to details', (done) => {
      ...
      effects.navigatedToDetailsPage$.pipe(take(2)).subscribe({
        ...
        complete: () => {
          expect(dispatched).toContainEqual(DocumentCreateOperationsActions.ensureReferenceDataLoaded())
  ...
  describe('loadDocumentById$', () => {
    it('should dispatch documentDetailsReceived on success', (done) => {
      ...
      effects.loadDocumentById$.pipe(take(1)).subscribe((action) => {
        expect(action).toEqual(DocumentDetailsActions.documentDetailsReceived({ details }))
        done()
      })
    ...
    it('should dispatch documentDetailsLoadingFailed on API error', (done) => {
      ...
      effects.loadDocumentById$.pipe(take(1)).subscribe((action) => {
        expect(action).toEqual(DocumentDetailsActions.documentDetailsLoadingFailed({ error }))
        done()
      })
    ...
    it('should use empty string when id is undefined in navigatedToDetailsPage', (done) => {
      ...
      actions$.next(DocumentDetailsActions.navigatedToDetailsPage({ id: undefined as any }))
    })
  })
  ...
  describe('cancelButtonClickedDirty$', () => {
    ...
    it('should dispatch cancelEditBackClicked when user cancels dialog', (done) => {
      portalDialogService.openDialog.mockReturnValue(of({ button: 'secondary' } as DialogState<any>))
  ...
  describe('saveButtonClicked$', () => {
    it('should dispatch updateDocumentCancelled when details have no id', (done) => {
      ...
      actions$.next(DocumentDetailsActions.saveButtonClicked({ details: {} as any }))
    })
    ...
    it('should dispatch updateDocumentSucceeded and documentDetailsReceived on save success', (done) => {
      ...
      effects.saveButtonClicked$.pipe(take(2)).subscribe({
        next: (a) => dispatched.push(a),
        complete: () => {
          expect(dispatched).toContainEqual(DocumentDetailsActions.updateDocumentSucceeded())
    ...
    it('should dispatch updateDocumentFailed and show error message on save error', (done) => {
      ...
      effects.saveButtonClicked$.pipe(take(1)).subscribe((action) => {
        expect(action).toEqual(DocumentDetailsActions.updateDocumentFailed({ error }))
  ...
  describe('deleteButtonClicked$', () => {
    ...
    it('should dispatch deleteDocumentFailed and show error message on delete error', (done) => {
      ...
      portalDialogService.openDialog.mockReturnValue(of({ button: 'primary' } as DialogState<any>))
      documentService.deleteDocumentById.mockReturnValue(throwError(() => error))

      effects.deleteButtonClicked$.pipe(take(1)).subscribe((action) => {
        expect(action).toEqual(DocumentDetailsActions.deleteDocumentFailed({ error }))
        expect(messageService.error).toHaveBeenCalledWith(
          expect.objectContaining({
  ...
  describe('deleteButtonClicked$ with null itemToDelete and confirmed dialog', () => {
    it('should throw error when itemToDelete is null and dialog is confirmed', (done) => {
      store.overrideSelector(documentDetailsSelectors.selectDetails, null as any)
      store.refreshState()
      portalDialogService.openDialog.mockReturnValue(of({ button: 'primary' } as DialogState<any>))
  ...
  describe('downloadAttachmentBlob$', () => {
    ...
    it('should dispatch attachmentDownloadFailed on download error', (done) => {
      fileHandlerService.downloadFile.mockReturnValue(throwError(() => 'download error'))
  ...
  describe('saveDownloadAttachment$', () => {
    it('should create an anchor element and trigger download', (done) => {
      ...
      const createElementSpy = jest.spyOn(document, 'createElement').mockReturnValue(anchorSpy as any)
      const appendChildSpy = jest.spyOn(document.body, 'appendChild').mockImplementation(() => anchorSpy as any)
  ...
  describe('displayError$', () => {
    it('should call messageService.error on documentDetailsLoadingFailed', (done) => {
      ...
      actions$.next(DocumentDetailsActions.documentDetailsLoadingFailed({ error: 'err' }))
    })
  ...
  describe('retryFileUpload$', () => {
    ...
    it('should dispatch retryFileUploadCanceled when dialog is canceled', (done) => {
      ...
      effects.retryFileUpload$.pipe(take(1)).subscribe((action) => {
        expect(action).toEqual(DocumentDetailsActions.retryFileUploadCanceled())
        done()
      })
    ...
    it('should dispatch retryFileUploadCanceled when dialog button is not primary', (done) => {
      portalDialogService.openDialog.mockReturnValue(of({ button: 'secondary', result: null } as any))
    ...
    it('should dispatch requestDocumentUploadUrls when dialog is confirmed with a file', (done) => {
      ...
      portalDialogService.openDialog.mockReturnValue(of({ button: 'primary', result: mockFile } as any))
    ...
    it('should dispatch retryFileUploadCanceled when dialog button is primary but result is null', (done) => {
      portalDialogService.openDialog.mockReturnValue(of({ button: 'primary', result: null } as any))
  ...
  describe('documentCreationSuccess$', () => {
    it('should dispatch navigatedToDetailsPage when documentCreationCompleted id matches current document', (done) => {
      ...
      effects.documentCreationSuccess$.pipe(take(1)).subscribe((action) => {
        expect(action).toEqual(DocumentDetailsActions.navigatedToDetailsPage({ id: 'doc-1' }))
        done()
      })
