...
export class DocumentDetailsComponent implements OnInit, OnDestroy {
  ...
  onFileUploadRetry({ id, fileName }: { id: string; fileName: string }) {
    this.store.dispatch(DocumentDetailsActions.retryFileUpload({ attachmentId: id, fileName }))
  }
        ...
        const actions: Action[] = [
          {
            ...
            actionCallback: () => {
              this.store.dispatch(DocumentDetailsActions.navigateBackButtonClicked())
            }
          },
          {
