...
describe('DocumentDetailsComponent', () => {
  ...
  beforeEach(async () => {
    ...
    store.overrideSelector(selectDocumentDetailsViewModel, baseDocumentDetailsViewModel)
    store.refreshState()
  ...
  it('should dispatch editButtonClicked when edit() is called', () => {
    ...
    component.edit()
    expect(store.dispatch).toHaveBeenCalledWith(DocumentDetailsActions.editButtonClicked())
  })
  ...
  it('should dispatch cancelButtonClicked with dirty=false when form is pristine', () => {
    jest.spyOn(store, 'dispatch')
    component.cancel()
    expect(store.dispatch).toHaveBeenCalledWith(DocumentDetailsActions.cancelButtonClicked({ dirty: false }))
  })

  it('should dispatch deleteButtonClicked when delete() is called', () => {
    jest.spyOn(store, 'dispatch')
    component.delete()
    expect(store.dispatch).toHaveBeenCalledWith(DocumentDetailsActions.deleteButtonClicked())
  })
  ...
  it('should dispatch cancelButtonClicked with dirty=true when form is dirty', () => {
    ...
    component.cancel()
    expect(store.dispatch).toHaveBeenCalledWith(DocumentDetailsActions.cancelButtonClicked({ dirty: true }))
  })
  ...
  it('should show back and edit actions and hide cancel and save when not in editMode', (done) => {
    ...
    store.refreshState()
    component.headerActions$.subscribe((actions) => {
      const back = actions.find((a) => a.labelKey === 'DOCUMENT_DETAILS.GENERAL.BACK')
      const edit = actions.find((a) => a.labelKey === 'DOCUMENT_DETAILS.GENERAL.EDIT')
      const cancel = actions.find((a) => a.labelKey === 'DOCUMENT_DETAILS.GENERAL.CANCEL')
      const save = actions.find((a) => a.labelKey === 'DOCUMENT_DETAILS.GENERAL.SAVE')
  ...
  describe('headerActions$ callbacks', () => {
    ...
    it('should call cancel() when cancel action callback is invoked', (done) => {
      jest.spyOn(store, 'dispatch')
      component.headerActions$.subscribe((actions) => {
        const cancelAction = actions.find((a) => a.labelKey?.includes('CANCEL'))
    ...
    it('should call delete() when delete action callback is invoked', (done) => {
      jest.spyOn(store, 'dispatch')
      component.headerActions$.subscribe((actions) => {
        const deleteAction = actions.find((a) => a.labelKey?.includes('DELETE'))
