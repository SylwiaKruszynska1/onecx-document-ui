...
describe('DocumentCreateOperationsEffects', () => {
  ...
  let mimeTypeService: jest.Mocked<SupportedMimeTypeControllerAPIService>
  ...
  beforeEach(async () => {
    ...
    } as unknown as jest.Mocked<SupportedMimeTypeControllerAPIService>
    ...
    await TestBed.configureTestingModule({
      providers: [
        ...
        { provide: SupportedMimeTypeControllerAPIService, useValue: mimeTypeService },
  ...
  describe('loadReferenceDataOnRouteEnter$', () => {
    it('should dispatch ensureReferenceDataLoaded when navigating to quick-upload', (done) => {
      effects.loadReferenceDataOnRouteEnter$.pipe(take(1)).subscribe((action) => {
        expect(action).toEqual(DocumentCreateOperationsActions.ensureReferenceDataLoaded())
        done()
      })
  ...
  describe('ensureReferenceDataLoaded$', () => {
    it('should fetch types and mimeTypes from API when store is empty', (done) => {
      ...
      store.overrideSelector(documentCreateOperationsSelectors.selectAvailableDocumentTypes, [])
      store.overrideSelector(documentCreateOperationsSelectors.selectAvailableMimeTypes, [])
      store.refreshState()
    ...
    it('should use cached types from store and only fetch mimeTypes', (done) => {
      ...
      store.overrideSelector(documentCreateOperationsSelectors.selectAvailableDocumentTypes, cachedTypes)
      store.overrideSelector(documentCreateOperationsSelectors.selectAvailableMimeTypes, [])
      store.refreshState()
    ...
    it('should use cached mimeTypes from store and only fetch documentTypes', (done) => {
      ...
      store.overrideSelector(documentCreateOperationsSelectors.selectAvailableDocumentTypes, [])
      store.overrideSelector(documentCreateOperationsSelectors.selectAvailableMimeTypes, cachedMimeTypes)
      store.refreshState()
      ...
      effects.ensureReferenceDataLoaded$.pipe(take(2)).subscribe({
        next: (a) => dispatched.push(a),
        complete: () => {
          expect(mimeTypeService.getAllSupportedMimeTypes).not.toHaveBeenCalled()
          expect(dispatched).toContainEqual(
            DocumentCreateOperationsActions.availableMimeTypesReceived({
    ...
    it('should dispatch loadReferenceDataFailed with null when thrown error has no message property', (done) => {
      documentTypeService.getAllTypesOfDocument.mockReturnValue(throwError(() => null))
      mimeTypeService.getAllSupportedMimeTypes.mockReturnValue(of([]) as any)
  ...
  describe('startDocumentCreation$', () => {
    ...
    it('should dispatch documentCreationFailed when createDocument fails', (done) => {
      ...
      effects.startDocumentCreation$.pipe(take(1)).subscribe((action) => {
        expect(action).toEqual(DocumentCreateOperationsActions.documentCreationFailed())
        done()
      })
  ...
  describe('documentCreatedSuccesfully$', () => {
    it('should dispatch requestDocumentUploadUrls with matched attachment IDs', (done) => {
      ...
      effects.documentCreatedSuccesfully$.pipe(take(1)).subscribe((action: any) => {
        expect(action.type).toBe(DocumentCreateOperationsActions.requestDocumentUploadUrls.type)
  ...
  describe('requestDocumentUploadUrls$', () => {
    it('should dispatch uploadAttachment for each presigned URL response', (done) => {
      ...
      effects.requestDocumentUploadUrls$.pipe(take(1)).subscribe((action: any) => {
        expect(action.type).toBe(DocumentCreateOperationsActions.uploadAttachment.type)
  ...
  describe('uploadAttachment$', () => {
    ...
    it('should dispatch attachmentUploadFailed when upload fails', (done) => {
      uploaderService.uploadAttachment.mockReturnValue(throwError(() => new Error('Upload error')))
  ...
  describe('trackUploadCompletion$', () => {
    it('should dispatch allAttachmentsUploaded when pendingAttachmentUploads reaches 0', (done) => {
      store.overrideSelector(documentCreateOperationsSelectors.selectPendingAttachmentUploads, 0)
      store.overrideSelector(documentCreateOperationsSelectors.selectSuccessfulAttachmentIds, ['att-1'])
      store.overrideSelector(documentCreateOperationsSelectors.selectFailedAttachmentIds, [])
      store.refreshState()
    ...
    it('should not dispatch when pendingAttachmentUploads is greater than 0', (done) => {
      store.overrideSelector(documentCreateOperationsSelectors.selectPendingAttachmentUploads, 2)
      store.refreshState()
  ...
  describe('allAttachmentsUploaded$', () => {
    ...
    it('should skip metadata update when no successful IDs and dispatch completed', (done) => {
      ...
      effects.allAttachmentsUploaded$.pipe(take(1)).subscribe((action) => {
        ...
        expect(action.type).toBe(DocumentCreateOperationsActions.documentCreationCompleted.type)
        done()
      })
    ...
    it('should dispatch documentCreationFinalStepFailed when metadata update fails', (done) => {
      documentService.updateAttachmentsMetadata.mockReturnValue(throwError(() => new Error('Metadata error')))
  ...
  describe('navigateToDetails$', () => {
    it('should navigate to document details after documentCreationCompleted', (done) => {
      ...
      effects.navigateToDetails$.pipe(take(1)).subscribe(() => {
        expect(navigateSpy).toHaveBeenCalledWith(expect.arrayContaining(['details', 'doc-1']))
        done()
      })
